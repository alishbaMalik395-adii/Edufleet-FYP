...
export default function StudentFaceDetectionScreen({ navigation, route }) {
  ...
  const onScanPress = async () => {
    try {
      ...
      formData.append('studentId', String(studentId || 'unknown'));

      const response = await fetch(BACKEND_URL + '/api/student/verify-face', {
        method: 'POST',
      ...
      if (response.ok && result2.success) {
        if (result2.isFirstTime) {
          ...
          Alert.alert(
            '✅ Face Registered!',
            'Your face has been saved. Next time you will be verified automatically.',
            [{
              text: 'Continue',
        ...
        if (response.status === 401) {
          Alert.alert(
            '❌ Access Denied',
            'Your face does not match this account. Please try again.',
            [{ text: 'Try Again', onPress: () => setStatusMsg('Press button to scan face') }]
          );
        } else {
          Alert.alert('Failed', result2.message || 'Please try again.');
        }
      }
    } catch (err) {
    ...
    <Animated.View style={[styles.container, { opacity: fadeAnim }]}>
      ...
      <View style={styles.topSection}>
        <View style={styles.topBadge}>
          <Text style={styles.topBadgeText}>🔐  STUDENT VERIFICATION</Text>
        </View>
        <Text style={styles.title}>Face Verification</Text>
        <Text style={styles.subtitle}>Center your face in the circle</Text>
      </View>
      ...
      <View style={styles.middleSection}>
        ...
        <View style={[styles.circleClip, { width: CIRCLE_SIZE, height: CIRCLE_SIZE }]}>
          ...
          {faceDetected && <View style={styles.faceFoundOverlay} />}
        </View>
...
const styles = StyleSheet.create({
  ...
  topSection: { alignItems: 'center', paddingTop: 60, paddingHorizontal: 24, zIndex: 10 },
  topBadge: {
    backgroundColor: 'rgba(33,150,243,0.2)',
    borderWidth: 1, borderColor: 'rgba(33,150,243,0.5)',
    borderRadius: 20, paddingHorizontal: 14, paddingVertical: 5, marginBottom: 12,
  },
  topBadgeText: { color: '#64B5F6', fontSize: 10, fontWeight: '700', letterSpacing: 2 },
  title: {
  ...
  rotatingRing: {
    ...
    borderWidth: 1.5, borderColor: 'rgba(33,150,243,0.4)',
    borderStyle: 'dashed',
  },
  pulseRing: {
    position: 'absolute',
    width: width * 0.79, height: width * 0.79,
    borderRadius: width * 0.395, borderWidth: 2,
  },
  pulseRingBlue: {
    borderColor: 'rgba(33,150,243,0.7)',
    shadowColor: '#2196F3', shadowOffset: { width: 0, height: 0 },
    shadowOpacity: 0.8, shadowRadius: 15, elevation: 15,
  },
  pulseRingGreen: {
    borderColor: '#00E676',
  ...
  circleClip: { borderRadius: 999, overflow: 'hidden', borderWidth: 3, borderColor: 'rgba(255,255,255,0.15)' },
  scanLine: {
    position: 'absolute', left: 0, right: 0, height: 2,
    backgroundColor: 'rgba(33,150,243,0.8)',
    shadowColor: '#2196F3', shadowOffset: { width: 0, height: 0 },
    shadowOpacity: 1, shadowRadius: 8, elevation: 8, zIndex: 5,
  },
  faceFoundOverlay: { ...StyleSheet.absoluteFillObject, backgroundColor: 'rgba(0,230,118,0.12)', zIndex: 4 },
  cornerDot: { position: 'absolute', width: 8, height: 8, borderRadius: 4, backgroundColor: '#2196F3' },
  ...
  statusPill: {
    ...
    borderRadius: 30, gap: 8, borderWidth: 1,
  },
  pillDark: { backgroundColor: 'rgba(0,0,0,0.8)', borderColor: 'rgba(255,255,255,0.1)' },
  pillGreen: { backgroundColor: 'rgba(0,230,118,0.15)', borderColor: '#00E676' },
  statusDot: { width: 8, height: 8, borderRadius: 4 },
  dotGray: { backgroundColor: '#546E7A' },
  dotGreen: { backgroundColor: '#00E676' },
  statusText: { color: '#FFFFFF', fontSize: 13, fontWeight: '600' },
  bottomSection: { paddingHorizontal: 24, paddingBottom: 40, paddingTop: 40, zIndex: 10 },
  scanBtn: {
  ...
  scanBtnDisabled: { backgroundColor: 'rgba(255,255,255,0.08)', shadowOpacity: 0, elevation: 0, borderColor: 'rgba(255,255,255,0.05)' },
  btnRow: { flexDirection: 'row', alignItems: 'center' },
  btnIcon: { fontSize: 22 },
  btnText: { color: '#FFFFFF', fontSize: 17, fontWeight: '700', letterSpacing: 0.5 },
  footer: { color: 'rgba(255,255,255,0.2)', fontSize: 11, textAlign: 'center', letterSpacing: 0.5 },
